package Test::Sync::IPC::Files;
use strict;
use warnings;

use base 'Test::Sync::IPC';
...
use Test::Sync::Util qw/try get_tid pkg_to_file/;
...
sub event_file {
    ...
    $self->abort("'$e' is not an event object!")
        unless $type->isa('Test::Sync::Event');
...
sub read_event_file {
    ...
    unless ($obj->isa('Test::Sync::Event')) {
        my $pkg  = blessed($obj);
        ...
        $self->abort("'$obj' is not a 'Test::Sync::Event' object")
            unless $obj->isa('Test::Sync::Event');
    }
...
sub waiting {
    my $self = shift;
    require Test::Sync::Event::Waiting;
    $self->send(
        GLOBAL => Test::Sync::Event::Waiting->new(
            debug => Test::Sync::DebugInfo->new(frame => [caller()]),
        )
    );
    return;
}
...
Test::Sync::IPC::Files - Temp dir + Files concurrency model.

=head1 DESCRIPTION

This is the default, and fallback concurrency model for L<Test::Sync>. This
...
or
    ...
    use Test::Sync ..., 'IPC' => ['+Test::Sync::IPC::Files'];

=head1 SEE ALSO

See L<Test::Sync::IPC> for methods.
