...
sub build_object {
    ...
    my $meta = $STRUCTS[-1];
    croak "object {} was called in void context with no build is on the stack!"
        unless $meta;

    if ($meta->isa('Test::Stream::DeepCheck::Array')) {
        $meta->add_element($obj);
    }
    elsif ($meta->isa('Test::Stream::DeepCheck::Hash')) {
        $meta->add_field($obj);
    }
    else {
        croak "object {} was called in void context for an invalid build object!";
    }
}
...
sub array(&) {
    ...
    my $meta = $STRUCTS[-1];
    croak "array {} was called in void context with no build is on the stack!"
        unless $meta;

    if ($meta->isa('Test::Stream::DeepCheck::Array')) {
        $meta->add_element($array);
    }
    elsif ($meta->isa('Test::Stream::DeepCheck::Hash')) {
        $meta->add_field($array);
    }
    else {
        croak "array {} was called in void context for an invalid build object!";
    }
}
...
sub hash(&) {
    ...
    my $meta = $STRUCTS[-1];
    croak "hash {} was called in void context with no build is on the stack!"
        unless $meta;

    if ($meta->isa('Test::Stream::DeepCheck::Array')) {
        $meta->add_element($hash);
    }
    elsif ($meta->isa('Test::Stream::DeepCheck::Hash')) {
        $meta->add_field($hash);
    }
    else {
        croak "hash {} was called in void context for an invalid build object!";
    }
}
...
sub check($;$) {
    ...
    my $array = $STRUCTS[-1];
    croak "check() was called in a void context, but no build is on the stack!"
        unless $array;

    croak "check() was called in a void context, but the top of the build stack is not an array!"
...
Used to build an array check. If called in void context it will attempt to add
it as an element to the parent build. If there is no parent build, or the
parent build is not an array, an exception will be thrown.
...
This quickly builds an L<Test::Stream::DeepCheck::Check> object for you. In
void context the check is added to the parent build, so long as it is an array
(added as an element) otherwise an exception is thrown. In any other context
the C<$check> object is returned.

This is useful for maing very specific checks of fields or method return
values:
...
=item convert($want, $debug, $strict, <$seen>)
...
=item build_object($class, $coderef, $caller)
