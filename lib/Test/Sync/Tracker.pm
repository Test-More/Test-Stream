...
use Test::Sync::Util qw/get_tid USE_THREADS pkg_to_file/;
...
sub _finalize {
    ...
    $self->{+STACK}     = Test::Sync::Stack->new;
    ...
    return unless $INC{'Test/Sync/IPC.pm'};
    $self->{+IPC} = Test::Sync::IPC->init;
}
...
L<Test::Sync>. A single global instance of this package is stored (and
obscured) by the L<Test::Sync> package.
