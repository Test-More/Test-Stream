...
my %LOADED = (
    map {
        require "Test/Sync/Event/$_.pm";
        my $pkg = "Test::Sync::Event::$_";
        ( $pkg => $pkg, $_ => $pkg )
...
sub ok {
    ...
    my $e = bless {
        debug => bless( {%{$self->{+DEBUG}}}, 'Test::Sync::DebugInfo'),
        pass  => $pass,
        name  => $name,
        $hub->_fast_todo,
    }, 'Test::Sync::Event::Ok';
...
sub _parse_event {
    ...
    confess "'$pkg' is not a subclass of 'Test::Sync::Event'"
        unless $pkg->isa('Test::Sync::Event');
...
Test::Sync::Context - Object to represent a testing context.

=head1 DESCRIPTION

The context object is the primary interface for authors of testing tools
written with L<Test::Sync>. The context object represents the context in
...
sending events to the correct L<Test::Sync::Hub> instance.
...
L<Test::Sync>. If you are maintaining your own L<Test::Sync::Stack>
...
=item Test::Sync::Context->ON_INIT(sub { ... }, ...)

=item Test::Sync::Context->ON_RELEASE(sub { ... }, ...)
...
This will return the L<Test::Sync::Stack> instance the context used to find
the current hub.

=item $hub = $ctx->hub()

This will return the L<Test::Sync::Hub> instance the context recognises as
the current one to which all events should be sent.

=item $dbg = $ctx->debug()

This will return the L<Test::Sync::DebugInfo> instance used by the context.
...
This will create an L<Test::Sync::Event::Ok> object for you. The diagnostics
...
Send an L<Test::Sync::Event::Note>. This event prints a message to STDOUT.

=item $event = $ctx->diag($message)

Send an L<Test::Sync::Event::Diag>. This event prints a message to STDERR.

=item $event = $ctx->plan($max)

=item $event = $ctx->plan(0, 'SKIP', $reason)

This can be used to send an L<Test::Sync::Event::Plan> event. This event
...
This sends an L<Test::Sync::Event::Bail> event. This event will completely
...
be the event package name with C<Test::Sync::Event::> left off, or a fully
...
or
    ...
    my $event = $ctx->send_event('+Test::Sync::Event::Ok', ...);
...
context for any hub or stack.
    ...
    Test::Sync::Context->ON_RELEASE(sub {
        my $ctx = shift;
        ...
    });
