...
sub drivers {
    unless(@DRIVERS) {
        ...
        push @DRIVERS => 'Test::Sync::IPC::Files';
    }
...
sub enable_polling {
    return if $POLLING++;
    require Test::Sync::Context;
    Test::Sync::Context->ON_INIT(sub { $_[0]->hub->cull });
}
...
Test::Sync::IPC - Base class for Test::Sync IPC drivers.

=head1 SYNOPSIS

    package Test::Sync::IPC::MyDriver;
...
initialization. Every time a context is obtained from L<Test::Sync::Context>
...
Test::Sync::IPC has an C<import()> method. All drivers inherit this import
