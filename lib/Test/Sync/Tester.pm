...
sub intercept(&) {
    ...
    my $ipc;
    if ($INC{'Test/Sync/IPC.pm'}) {
        my ($driver) = Test::Sync::IPC->drivers;
        $ipc = $driver->new;
    }

    my $hub = Test::Sync::Hub::Interceptor->new(
        ipc => $ipc,
        no_ending => 1,
    );
    ...
    die $err unless $ok
        || (blessed($err) && $err->isa('Test::Sync::Hub::Interceptor::Terminator'));
...
Test::Sync::Tester - Essential basic tools for testing Test::Sync, and Test::Sync based
tools.
...
This library exports some essential tools needed for Test::Sync, and frameworks
built on it, to test themselves. These tools are bare-bones and intended only
for Test::Sync, and frameworks built on top of it. Think of this as a testing
