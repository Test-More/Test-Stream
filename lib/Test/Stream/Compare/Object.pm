...
sub deltas {
    ...
    push @deltas => $meta->deltas($got, $convert, $seen) if $meta;
    ...
    for my $call (@{$self->{+CALLS}}) {
        ...
        elsif ($exists || $check->isa('Test::Stream::Compare::DNE')) {
    ...
    if ($refcheck->verify($got)) {
        push @deltas => $refcheck->deltas($got, $convert, $seen);
    }
    else {
        push @deltas => $self->delta_class->new(
            verified => undef,
