...
tests verify => sub {
    ...
    ok(!$one->verify(undef), "undef is not a hashref");
    ok(!$one->verify(1),     "1 is not a hashref");
    ok(!$one->verify([]),    "An arrayref is not a hashref");

    ok($one->verify({}), "got a hashref");
};
...
tests deltas => sub {
    ...
    is(
        [$one->deltas({a => 1, b => 2, c => 3}, $convert, {})],
        [],
        "No deltas, perfect match"
    );

    is(
        [$one->deltas({a => 1, b => 2, c => 3, e => 4, f => 5}, $convert, {})],
        [],
    ...
    is(
        [$one->deltas({a => 1, b => 1, c => 1}, $convert, {})],
        [
            {
                children => [],
                verified => F(),
    ...
    like(
        [$one->deltas({a => 1, b => 2, c => 3, x => 'oops'}, $convert, {})],
        [
            {
                verified => F(),
                id       => [HASH => 'x'],
